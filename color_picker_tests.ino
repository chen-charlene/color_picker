...
void testAllMain() {
  ...
  if (!testUpdateInputs()) {
    counter--;
    Serial.println("updateInputs - FAILED");
  } 
  if (!testButtonISR()) {
    counter--;
...
bool testUpdateInputs() {
  // tests to make sure that the output of updateInputs correctly passes the rgb, combines it with the opacity, 
...
bool testTransition(state startState, state endState, state_inputs testStateInputs, state_vars startStateVars, state_vars endStateVars, bool verbos) { 
  ...
  state resultState = updateFSM(startState, testStateInputs.numButtons, testStateInputs.mils);
  ...
  if (! verbos) {
    ...
    Serial.println(sToPrint);
    sprintf(sToPrint, "Inputs: mils %ld | numButtons %d ", testStateInputs.mils, testStateInputs.numButtons);
    Serial.println(sToPrint);
